This file is 395 lines long; read all of them.

# web-poet Reference

Concise reference for writing web-poet page objects. For full docs see
https://web-poet.readthedocs.io/llms.txt
...
- Choosing the container: pick the *smallest* repeated element that contains all
  of one item's fields, and sanity-check that the number of containers on the page
  matches the number of items you expect
...
- `inputs/` = the page object's serialized dependencies — one or more files per
  dependency (e.g. `HttpResponse-body.html`, `HttpResponse-info.json`)
- `output.json` = expected result of `to_item()`
- `meta.json` = optional; set `frozen_time` (an ISO-8601 timestamp *with*
  timezone, e.g. `"2001-01-01T11:00:00+01:00"`) for page objects that produce
  time-dependent fields

### Generating a fixture

Don't hand-write the `inputs/` files. Once the page object exists and is
importable, generate the fixture with scrapy-poet's `savefixture` command — it
fetches the URL, runs `to_item()`, and writes the whole fixture directory for
you (see `scrapy-poet.md` → Testing fixtures for the command). Repeat with
different URLs to add `test-2`, `test-3`, …, covering the variations that matter
(e.g. an in-stock vs. an out-of-stock product).

### Generating a fixture from local inputs (no URL)

When you already have the page HTML saved locally (sample files, no live URL to
fetch), don't call `savefixture` — build the page object's input in memory and
save the fixture with web-poet's own serializer, `Fixture.save`. This is the
same serializer `savefixture` uses (never hand-write the `inputs/` files), and
it works for both `HttpResponse` and `BrowserResponse` pages:

```python
from web_poet import HttpResponse            # BrowserPage → BrowserResponse(url=..., html=...)
from web_poet.testing import Fixture

resp = HttpResponse(url="https://example.com/a",
                    body=open("samples/page-a.html", "rb").read())
Fixture.save(
    "fixtures/my_project.pages.example_com.ProductPage",  # fixtures/<class path>
    inputs=[resp],
    item={"name": "...", "price": "..."},   # the *expected* values, not to_item()'s output
    fixture_name="page-a",
)
```

Pass the hand-verified expected values as `item=` — that becomes `output.json`,
so `pytest` fails if the page object doesn't reproduce them. Repeat per sample
(`fixture_name="page-b"`, …). Within the `scrape` workflow, the bundled
`make_fixtures.py` helper does exactly this from a spec's captured pages.

### Setting expected values

`savefixture` seeds `output.json` with whatever the page object *currently*
produces — that is a recording, not a check. Open `output.json` and correct each
field to the value the page *should* yield. That hand-verified `output.json` is
what turns the fixture into a regression test: pytest fails when the page object
later drifts from it.

### Running and updating

```bash
uv run --with pytest python -m pytest fixtures/            # run all fixtures
uv run --with pytest python -m pytest FIXTURE_PATH         # run one fixture
uv run python -m web_poet.testing rerun FIXTURE_PATH       # print current output
uv run python -m web_poet.testing rerun FIXTURE_PATH --fields price,stock
```

After you change a page object, `rerun` prints its current output so you can diff
against `output.json` — it does **not** rewrite the file, so copy the correct
values in by hand. If the saved input HTML itself has gone stale, delete the
fixture directory and re-`savefixture` it.
