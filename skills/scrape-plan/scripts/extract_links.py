...
Usage:
    uv run --python 3.14 extract_links.py PAGE.html [--css SELECTOR] [--base-url URL] [--link-limit LIMIT]
    uv run --python 3.14 extract_links.py PAGE.html --group          # group by path
    uv run --python 3.14 extract_links.py *.html --group             # multiple files, numbered globally

    # Multiple files with per-file base URLs from meta.json:
    uv run --python 3.14 extract_links.py pages/list-1/rendered.html pages/list-2/rendered.html --group --base-url-from-meta

Outputs JSON lines to stdout. With --group, outputs one numbered object per path group:
    0: {"path": "article.product_pod > h3 > a", "count": 20, "links": [...], "file": "list-1"}
    1: {"path": "aside.sidebar > ul.nav > li > a", "count": 12, "links": [...], "file": "list-1"}

The "file" key labels the page directory the group came from, so that
apply_classification.py writes each links.json next to its source page.

Without --group, outputs one object per link:
    {"href": "/product/123", "text": "Widget X", "path": "div.products > div.card > h3 > a", "file": "detail-1"}

--link-limit can be used to limit the number of links per path group.
--counts prints a link count per path group, which is enough to compare the
link coverage of two pages without reading the links themselves.

Timing info and --counts output are printed to stderr.
"""
...
def main():
    ...
    parser.add_argument(
        "--counts",
        action="store_true",
        help="Print a link count per path group to stderr (implies --group)",
    )
    parser.add_argument(
        "--depth",
        type=int,
    ...
    if args.counts:
        for i, group in enumerate(all_groups):
            label = f"{group['file']} " if "file" in group else ""
            print(f"{i}: {group['count']} links {label}{group['path']}", file=sys.stderr)
